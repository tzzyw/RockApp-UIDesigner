...
namespace Rock.RunCommon
{
    ...
    {
        public DataTable GetDataList(string cmdText)
        {
            return Gateways.Default.Db.ExecuteDataSet(CommandType.Text, cmdText).Tables[0];
        }

        public Customer GetCustomer(int customerID)
        {
            return Gateways.Default.Find<Customer>(customerID);
        }

        public void SaveCustomer(Customer customer)
        {
            Gateways.Default.Save<Customer>(customer);
        }
        public int ExecuteCount(string cmdText)
        {
            object o = Gateways.Default.Db.ExecuteScalar(CommandType.Text, cmdText);
            if (o != DBNull.Value)
            {
        ...
        private object Add(object a, object b)
        {
            ...
            decimal d1 = 0, d2 = 0;
            decimal.TryParse(a.ToString(), out d1);
            decimal.TryParse(b.ToString(), out d2);
            return d1 + d2;
        }
