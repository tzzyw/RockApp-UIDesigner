...
$(function () {
    ...
    //加载动态脚本
    var jsTypes = "ISystemService,IBusinessService,DataTable,DataRow,DataColumn,Measure";
        ...
        if (ISystemService.execQuery.success) {
            (function (e) {
                if (e != null) {
                    ...
                    for (var i = 0; i < rowLength; i++) {
                        var rowResult = rows[i].values;
                        $("#comboplatform").append("<option value='" + rowResult[0].value + "'>" + rowResult[0].value + "</option>")
                    }
                }
            }(ISystemService.execQuery.resultValue));
        }
    ...
    toolBar.addText("createTimeBegin", null, "过磅日期");
    ...
    toolBar.attachEvent("onClick", function (id) {
        switch (id) {
            ...
            case "add":
                ...
                $("#txtvehicleNum").val("");
                $("#txtshiper").val("");
                $("#txtcreateTime").val(serverDate);
                $("#comboplatform").get(0).selectedIndex = 0;
            ...
            case "modify":
                ...
                if (checked != "") {
                    if (checked.indexOf(',') == -1) {
                        ...
                        rock.setSelectItem("comboplatform", measure.platform, "text");
    ...
    $("#btn_Save").click(function () {
        if (!$("#txtmeasureNum").validate("required", "计量单编号")) {
            return false;
        }
        ...
        if (!$("#txtvehicleNum").validate("required", "车号")) {
            return false;
        }
        if (!$("#txtplanQuantity").validate("number", "计划数量")) {
            return false;
        }
        if (!$("#txtcreateTime").validate("date", "创建时间")) {
            return false;
        }
        ...
        measure.createTime = $("#txtcreateTime").val();
        ...
        measure.platform = $("#comboplatform").val();
        ...
        if (editState == "add") {
            ...
            if (ISystemService.addDynObject.success) {
                (function (e) {
                    ...
                    dictData.data.push($("#comboplatform").val()); 
                ...
                (function (e) {
                    for (var i = 0; i < dictDataList.rows.length; i++) {
                        if (dictDataList.rows[i].id == measure.measureID) {
                            ...
                            dictDataList.rows[i].data[8] = $("#comboplatform").val(); 
    ...
    function customerComplete(searchCode) {
        ISystemService.execQuery.sqlString = "select top 15 [Customer].[CustomerID], [Customer].[CustomerName] from [Customer] where Available = '1' and ForPurchase = '1' and ([CustomerName] like  '%" + $("#txtcustomer").val() + "%' or [SearchCode] like  '%" + $("#txtcustomer").val() + "%')";
        rock.AjaxRequest(ISystemService.execQuery, rock.exceptionFun);
        if (ISystemService.execQuery.success) {
            (function (e) {
