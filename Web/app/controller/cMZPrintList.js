$(function () {
    ...
    //加载动态脚本
    var jsTypes = "ISystemService,ICommonService,DataTable,DataRow,DataColumn,Measure";
    $.getScript('/LoadDomainJS.ashx?JsTypes=' + jsTypes, function () {
        ...
        sqlStr = "select top 100 [Measure].[MeasureID], [Customer].[CustomerName], [Material].[MaterialName], [Measure].[grossTime], [Measure].[vehicleNum], [Measure].[netWeight] from [Measure] join [Customer] on [Measure].[customerID] = [Customer].[customerID] join [Material] on [Measure].[materialID] = [Material].[materialID] and [Measure].[Platform] = '" + platName + "'";
    ...
    listGrid.attachEvent("onRowSelect", function (rowID, cIndex) {
        window.open("CMZPrint.html?id=" + rowID);
        return true;
    });
    listGrid.init();
