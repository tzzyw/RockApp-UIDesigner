...
$(function () {
    ...
    $.getScript('/LoadDomainJS.ashx?JsTypes=' + jsTypes, function () {
        ...
        //处理初始化加载数据
        sqlStr = "select top 100 [LadeBill].[LadeBillID], [Customer].[CustomerName], [Material].[MaterialName], [LadeBill].[plateNumber], Convert(decimal(18,2),[LadeBill].[planQuantity]), Convert(decimal(18,2),[LadeBill].[actualQuantity]), convert(nvarchar(10),[LadeBill].[BillingTime],120) as BillingTime, convert(nvarchar(10),[LadeBill].[ladeDate],120) as ladeDate, [LadeBill].[state] from [LadeBill] join [Customer] on [LadeBill].[customerID] = [Customer].[customerID] join [Material] on [LadeBill].[materialID] = [Material].[materialID] and [LadeBill].[Agent] = '" + decodeURIComponent($.cookie('userTrueName')) + "'";
        if ($('#kp').attr('checked')) {
    ...
    toolBar.attachEvent("onClick", function (id) {
        switch (id) {
            ...
            case "query":
                ...
                sqlStr = "select [LadeBill].[LadeBillID], [Customer].[CustomerName], [Material].[MaterialName], [LadeBill].[plateNumber], Convert(decimal(18,2),[LadeBill].[planQuantity]), Convert(decimal(18,2),[LadeBill].[actualQuantity]), convert(nvarchar(10),[LadeBill].[BillingTime],120) as BillingTime, convert(nvarchar(10),[LadeBill].[ladeDate],120) as ladeDate, [LadeBill].[state] from [LadeBill] join [Customer] on [LadeBill].[customerID] = [Customer].[customerID] join [Material] on [LadeBill].[materialID] = [Material].[materialID] and [LadeBill].[Agent] = '" + decodeURIComponent($.cookie('userTrueName')) + "'";
                if ($('#kp').attr('checked')) {
                    sqlStr += " and [LadeBill].[billingTime] between '" + toolBar.getValue("beginbillingTime") + " 0:0:0' AND '" + toolBar.getValue("endbillingTime") + " 23:59:59' ";
                }
                else {
    ...
    listGrid.setHeader("序号,,客户名称,产品名称,车号,计划量,实际量,开票日期,提货日期,状态");
    listGrid.setInitWidths("40,0,190,120,120,70,70,80,80,*");
