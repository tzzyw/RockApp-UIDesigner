...
$(function () {
    ...
    listGrid.setHeader("序号,,查看明细,计量情况,客户名称,产品名称,单价,计划量,实际量,车船号,提货日期,到站名称,状态,");
    listGrid.setInitWidths("40,0,65,65,190,120,80,70,70,120,80,150,*,0");
    listGrid.setColAlign("center,left,center,center,left,left,left,left,left,left,left,left,left,left");
    listGrid.setColSorting("na,na,na,na,str,str,str,str,str,str,str,str,str,str");
    listGrid.setColTypes("cntr,ro,link,link,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro");
    ...
    function getDataList() {
        ...
        sqlStr = "select [LadeBill].[LadeBillID], [Customer].[CustomerName], [Material].[MaterialName], case when [LadeBill].[ContractID] > 0 then Convert(decimal(18,2),[LadeBill].[ContractPrice]) else Convert(decimal(18,2),[LadeBill].[quotedPrice]) end, Convert(decimal(18,2),[LadeBill].[planQuantity]), Convert(decimal(18,2),[LadeBill].[actualQuantity]), [LadeBill].[plateNumber], convert(nvarchar(10),[LadeBill].[ladeDate],120) as ladeDate, [LadeBill].[destination], [LadeBill].[state], [LadeBill].[ContractID] from [LadeBill] join [Customer] on [LadeBill].[customerID] = [Customer].[customerID] join [Material] on [LadeBill].[materialID] = [Material].[materialID] and [LadeBill].[Agent] = '" + decodeURIComponent($.cookie('userTrueName')) + "'";
        if ($('#kp').attr('checked')) {
            ...
            (function (e) {
                ...
                if (e != null) {
                    ...
                    for (var i = 0; i < rowLength; i++) {
                        ...
                        if (parseInt(rowResult[10].value,10) > 0) {
                            listdata.data[2] = "查看明细^javascript:billc(" + rowResult[0].value + ");^_self";
                        }
                        else {
                            listdata.data[2] = "查看明细^javascript:billt(" + rowResult[0].value + ");^_self";
                        }
                        listdata.data[3] = "计量情况^javascript:alert(1);^_self";
                    ...
                    listdata.data[5] = "";
                    listdata.data[6] = "";
                    listdata.data[7] = Number(planquantity).toFixed(2).toString();
                    if (actquantity > 0) {
    ...
    billc = function (id) {
        window.parent.openBillManage(id, "LadeBillByContract", "合同提货单");
    }
    billt = function (id) {
        window.parent.openBillManage(id, "LadeBill", "挂牌价提货单");
    }
})