...
$(function () {
    ...
    $.getScript('/LoadDomainJS.ashx?JsTypes=' + jsTypes, function () {
        ...
        sqlStr = "select top 100 [Measure].[MeasureID], [Material].[MaterialName], [Customer].[CustomerName], [Measure].[heavyOperator], convert(nvarchar(10),[Measure].[createTime],120) as createTime, [Measure].[vehicleNum], [Measure].[planQuantity], [Measure].[gross], [Measure].[tare], [Measure].[netWeight], [Measure].[shiper], convert(nvarchar(10),[Measure].[grossTime],120) as grossTime, [Measure].[platform], [Measure].[sealNum], [Measure].[deliveryTank], [Measure].[comment] from [Measure] join [Material] on [Measure].[materialID] = [Material].[materialID] join [Customer] on [Measure].[customerID] = [Customer].[customerID] ";
    ...
    toolBar.addText("createTimeBegin", null, "计量日期");
    toolBar.addInput("begincreateTime", null, "", 65);
    toolBar.addText("创建时间End", null, "-");
    toolBar.addInput("endcreateTime", null, "", 65);
    ...
    toolBar.addText("txtshiperSearch", null, "运输单位");
    toolBar.addInput("xt", null, "", 100);
    ...
    toolBar.attachEvent("onClick", function (id) {
        switch (id) {
            case "query":
                ...
                if (toolBar.getValue("txtshiperSearch") != "") {
                    sqlStr += " and [Measure].[Shiper] like '%" + toolBar.getValue("txtshiperSearch") + "%'";
                }
    ...
    listGrid.setHeader("序号,,物料名称,客户名称,重车司磅员,创建时间,车号,计划数量,毛重,皮重,净重,运输单位,毛重时间,站台,封签号,发货罐体,备注");
    listGrid.setInitWidths("40,0,90,120,90,110,80,80,80,80,80,100,110,100,100,80,80");
    listGrid.setColAlign("center,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left");
    listGrid.setColSorting("na,na,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str");
    listGrid.setColTypes("cntr,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro");
