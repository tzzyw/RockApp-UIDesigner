...
$(function () {
    ...
    toolBar.addText("tareTimeBegin", null, "毛重时间");
    toolBar.addInput("begintareTime", null, "", 75);
    toolBar.addText("毛重时间End", null, "-");
        ...
        switch (id) {
            ...
            case "query":
                ...
                if (!rock.chkdate(toolBar.getValue("begintareTime"), "-")) {
                    alert("起始毛重时间格式不正确,正确格式为 2010-10-10！");
                    return false;
                }
                ...
                if (!rock.chkdate(toolBar.getValue("endtareTime"), "-")) {
                    alert("截止毛重时间格式不正确,正确格式为 2010-10-10！");
                    return false;
                }
                ...
                sqlStr += " and [Measure].[GrossTime] between '" + toolBar.getValue("begintareTime") + " 0:0:0' AND '" + toolBar.getValue("endtareTime") + " 23:59:59' ";
    ...
    listGrid.attachEvent("onRowSelect", function (rowID, cIndex) {
        window.open("WeightPrint_Purchase.html?id=" + rowID + "&act=Reprint");
        return true;
    });
    listGrid.init();
