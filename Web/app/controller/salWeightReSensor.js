...
$(function () {
    ...
    toolBar.addButton("repeal", null, "弃审");
    ...
    listGrid.setHeader("选择,,计量单编号,客户名称,物料名称,车号,净重");
    listGrid.setInitWidths("40,0,120,200,150,120,*");
    ...
    $("#btn_Save").click(function () {
        IBusinessService.auditMeasure.measureID = measure.measureID;
        IBusinessService.auditMeasure.type = "弃审";
        rock.AjaxRequest(IBusinessService.auditMeasure, rock.exceptionFun);
        if (IBusinessService.auditMeasure.success) {
            $("#txtcustomer").val("");
    ...
    function getSensorList() {
        sqlStr = "select top 100 [Measure].[MeasureID], [Measure].[measureNum], [Customer].[CustomerName], [Material].[MaterialName], [Measure].[vehicleNum], [Measure].[netWeight] from [Measure] join [Customer] on [Measure].[customerID] = [Customer].[customerID] join [Material] on [Measure].[materialID] = [Material].[materialID] and [State] = '计量提交' and MeasureType = '销售称重' and Closed = '0' and Platform='" + platName + "' ";
