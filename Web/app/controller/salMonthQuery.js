...
$(function () {
    ...
    listGrid.setHeader("序号,,产品名称,客户名称,单价,计划量,金额,实际量,金额,开票日期,提货日期,经办人");
    listGrid.setInitWidths("40,0,120,190,80,70,70,70,70,75,75,*");
    ...
    listGrid.setColTypes("cntr,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro");
    ...
    function getDataList() {
        ...
        sqlStr = "select [LadeBill].[LadeBillID], [Material].[MaterialName], [Customer].[CustomerName], case when [LadeBill].[ContractID] > 0 then Convert(decimal(18,2),[LadeBill].[ContractPrice]) else Convert(decimal(18,2),[LadeBill].[quotedPrice]) end, Convert(decimal(18,2),[LadeBill].[planQuantity]), Convert(decimal(18,2),[LadeBill].[PlanTotal]), Convert(decimal(18,2),[LadeBill].[actualQuantity]), Convert(decimal(18,2),[LadeBill].[ActualTotal]), convert(nvarchar(10),[LadeBill].[BillingTime],120) as BillingTime, convert(nvarchar(10),[LadeBill].[ladeDate],120) as ladeDate, [LadeBill].[Agent] from [LadeBill] join [Customer] on [LadeBill].[customerID] = [Customer].[customerID] join [Material] on [LadeBill].[materialID] = [Material].[materialID] and [LadeBill].[Agent] = '" + decodeURIComponent($.cookie('userTrueName')) + "'";
        if ($('#kp').attr('checked')) {
                ...
                if (e != null) {
                    ...
                    for (var i = 0; i < rowLength; i++) {
                        ...
                        if (rowResult[5].value) {
                            plantotal += parseFloat(rowResult[5].value);
                        }
                        if (rowResult[6].value) {
                            actquantity += parseFloat(rowResult[6].value);
                        }
                        if (rowResult[7].value) {
                            acttotal += parseFloat(rowResult[7].value);
                        }
                    ...
                    if (plantotal > 0) {
                        listdata.data[6] = Number(plantotal).toFixed(3).toString();
                    }
                    if (actquantity > 0) {
                    ...
                    if (acttotal > 0) {
                        listdata.data[8] = Number(acttotal).toFixed(3).toString();
                    }
