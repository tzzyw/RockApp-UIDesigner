...
$(function () {
    ...
    $.getScript('/LoadDomainJS.ashx?JsTypes=' + jsTypes, function () {
        ...
        //处理初始化加载数据
        sqlStr = "select top 100 [LadeBill].[LadeBillID], [Customer].[CustomerName], [Material].[MaterialName], Convert(decimal(18,2),[LadeBill].[planQuantity]), Convert(decimal(18,2),[LadeBill].[actualQuantity]), convert(nvarchar(10),[LadeBill].[BillingTime],120), convert(nvarchar(10),[LadeBill].[ladeDate],120) as ladeDate, [LadeBill].[destination], [LadeBill].[state] from [LadeBill] join [Customer] on [LadeBill].[customerID] = [Customer].[customerID] join [Material] on [LadeBill].[materialID] = [Material].[materialID] and [LadeBill].[Agent] = '" + decodeURIComponent($.cookie('userTrueName')) + "'";
    ...
    toolBar.attachEvent("onClick", function (id) {
        switch (id) {
            ...
            case "query":
                ...
                sqlStr = "select [LadeBill].[LadeBillID], [Customer].[CustomerName], [Material].[MaterialName], Convert(decimal(18,2),[LadeBill].[planQuantity]), Convert(decimal(18,2),[LadeBill].[actualQuantity]), [LadeBill].[plateNumber], convert(nvarchar(10),[LadeBill].[ladeDate],120) as ladeDate, [LadeBill].[destination], [LadeBill].[state] from [LadeBill] join [Customer] on [LadeBill].[customerID] = [Customer].[customerID] join [Material] on [LadeBill].[materialID] = [Material].[materialID] and [LadeBill].[Closed] = '0' and [LadeBill].[Agent] = '" + decodeURIComponent($.cookie('userTrueName')) + "'";
                sqlStr += " and [LadeBill].[billingTime] between '" + toolBar.getValue("beginbillingTime") + " 0:0:0' AND '" + toolBar.getValue("endbillingTime") + " 23:59:59' ";
    ...
    listGrid.setHeader("序号,,打印,客户名称,产品名称,计划量,实际量,开票日期,提货日期,到站名称,状态");
    listGrid.setInitWidths("40,0,40,190,120,70,70,80,80,150,*");
    listGrid.setColAlign("center,left,center,left,left,left,left,left,left,left,left");
    listGrid.setColSorting("na,na,na,na,str,str,str,str,str,str,str");
    listGrid.setColTypes("cntr,ro,img,ro,ro,ro,ro,ro,ro,ro,ro");
    listGrid.attachEvent("onRowSelect", function (rowID, cIndex) {
        window.open("LadeBillPrint.html?id=" + rowID);
        return true;
    });
    listGrid.init();
