...
$(function () {
    ...
    $.getScript('/LoadDomainJS.ashx?JsTypes=' + jsTypes, function () {
        ...
        sqlStr = "select top 100 [PriceApply].[PriceApplyID], [PriceApply].[priceApplyNum], [PriceApply].[productName], convert(nvarchar(10),[PriceApply].[createDate],120) as createDate, case [PriceApply].[Over50] when '1' then '是' else '否' end, [PriceApply].[state] from [PriceApply] where [state] = '已退回'";
    ...
    toolBar.attachEvent("onClick", function (id) {
        switch (id) {
            ...
            case "query":
                ...
                sqlStr = "select [PriceApply].[PriceApplyID], [PriceApply].[priceApplyNum], [PriceApply].[productName], convert(nvarchar(10),[PriceApply].[createDate],120) as createDate, case [PriceApply].[Over50] when '1' then '是' else '否' end, [PriceApply].[state] from [PriceApply] where [state] = '已退回'";

                sqlStr += " and [PriceApply].[createDate] between '" + toolBar.getValue("begincreateDate") + " 0:0:0' AND '" + toolBar.getValue("endcreateDate") + " 23:59:59' ";
                ...
                break;
            case "add":
                window.parent.openBillManage(null, "PriceApply", "产品调价申请维护");
                break;
    ...
    masterGrid.setHeader("序号,,修改,申请单编号,产品名称,创建日期,累计幅度大于50元,状态");
    ...
    masterGrid.attachEvent("onRowSelect", function (rowID, cIndex) {
        ...
        if (cIndex == 2) {
            var pageState = masterGrid.cells(rowID, 7).getValue();
            var state = null;
            //数据库状态
            ...
            if (pageState == "已创建" || pageState == "已提交") {
                if (state == "已创建" || state == "已提交") {
                    window.parent.openBillManage(rowID, "PriceApply", "产品调价申请维护");
                }
                else {
                    alert("产品调价申请单当前数据库中的状态为:" + state + ",不可以修改");
                }
            }
        }
    });
