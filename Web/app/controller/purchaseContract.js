...
$(function () {
    //初始化系统通用变量
    var toolBar, listGrid, editState, editForm, dictDataList, sqlStr, serverDate, dhxLayout, divUploadLayout, uploadFile, documentToolBar, documentGrid,
    ...
    $.getScript('/LoadDomainJS.ashx?JsTypes=' + jsTypes, function () {
        ...
        sqlStr = "select top 100 [Contract].[ContractID], [Contract].[contractNum], [Customer].[CustomerName], convert(nvarchar(10),[Contract].[signDate],120) as signDate, [Material].[MaterialName], [Contract].[price], [Contract].[quantity], [Contract].[State], case closed when '0' then '否' when '1' then '是' end  , [Contract].[Causes] from [Contract] join [Customer] on [Contract].[customerID] = [Customer].[customerID] join [Material] on [Contract].[materialID] = [Material].[materialID] and [Contract].[ContractType] = '采购' and [Contract].[Agent] = '" + decodeURIComponent($.cookie('userTrueName')) + "'";
