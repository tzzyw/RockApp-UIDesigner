...
$(function () {
    ...
    $("#btnidentity").click(function () {
        ...
        $("#txtiCCardNumber").val("153");
        $("#txtICCustomerCode").val("0002");
        $("#txtcustomer").val("南京海悦");
        if (audited()) {
            alert("客户身份验证通过!");
        }
    ...
    function audited() {
        //获取待称重车辆
        if ($("#txtplatName").val() != "") {
            sqlStr = "select Measure.MeasureID, Customer.CustomerName, Measure.VehicleNum, Material.MaterialName from Measure, Customer, Material where Measure.CustomerID = Customer.CustomerID and Measure.MaterialID = Material.MaterialID and Measure.MeasureType = '采购称重' and Measure.Closed = '0' and Measure.createTime = convert(varchar(8),getdate(),112) ";
            sqlStr += " and Measure.Audit = '0' and Measure.State = '已创建' and Platform = '" + platName + "' and Customer.CustomerCode='" + $("#txtICCustomerCode").val() + "'";
