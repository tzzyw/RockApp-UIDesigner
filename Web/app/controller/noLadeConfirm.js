...
$(function () {
    ...
    toolBar.addButton("confirm", null, "反确认");
    toolBar.attachEvent("onClick", function (id) {
        switch (id) {
            ...
            case "confirm":
                ...
                if (confirm("您确定要反确认所选的提货单吗?")) {
                    var rowids = checked.split(',');
                    for (var i = 0; i < rowids.length; i++) {
                        IBusinessService.reConfirmLadeBill.ladeBillID = rowids[i];
                        rock.AjaxRequest(IBusinessService.reConfirmLadeBill, rock.exceptionFun);
                        if (!IBusinessService.reConfirmLadeBill.success) {
                            getDataList();
                            alert("提货单反确认出错,请检查");
                            return;
                        }
    ...
    function getDataList() {
        ...
        sqlStr = "select [LadeBill].[LadeBillID], [Customer].[CustomerName], [Material].[MaterialName], [LadeBill].[MaterialLevel], Convert(decimal(18,2),[LadeBill].[PlanQuantity]), Convert(decimal(18,2),[LadeBill].[actualQuantity]), [LadeBill].[plateNumber], convert(nvarchar(10),[LadeBill].[BillingTime],120), convert(nvarchar(10),[LadeBill].[ladeDate],120) as ladeDate, [LadeBill].[destination] from [LadeBill] join [Customer] on [LadeBill].[customerID] = [Customer].[customerID] join [Material] on [LadeBill].[materialID] = [Material].[materialID] join Measure on [LadeBill].MeasureID = Measure.MeasureID and [Measure].[state] = '已确认' and [LadeBill].[Agent] = '" + decodeURIComponent($.cookie('userTrueName')) + "'";
