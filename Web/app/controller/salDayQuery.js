...
$(function () {
    ...
    listGrid.setHeader("序号,,客户名称,产品名称,单价,计划量,实际量,开票日期,提货日期,到站名称,状态");
    listGrid.setInitWidths("40,0,190,120,80,70,70,120,80,150,*");
    ...
    function getDataList() {
        ...
        sqlStr = "select [LadeBill].[LadeBillID], [Customer].[CustomerName], [Material].[MaterialName], case when [LadeBill].[ContractID] > 0 then Convert(decimal(18,2),[LadeBill].[ContractPrice]) else Convert(decimal(18,2),[LadeBill].[quotedPrice]) end, Convert(decimal(18,2),[LadeBill].[planQuantity]), Convert(decimal(18,2),[LadeBill].[actualQuantity]), convert(nvarchar(10),[LadeBill].[BillingTime],120) as BillingTime, convert(nvarchar(10),[LadeBill].[ladeDate],120) as ladeDate, [LadeBill].[destination], [LadeBill].[state] from [LadeBill] join [Customer] on [LadeBill].[customerID] = [Customer].[customerID] join [Material] on [LadeBill].[materialID] = [Material].[materialID] and [LadeBill].[Agent] = '" + decodeURIComponent($.cookie('userTrueName')) + "'";
        if ($('#kp').attr('checked')) {
