...
$(function () {
    ...
    $.getScript('/LoadDomainJS.ashx?JsTypes=' + jsTypes, function () {
        ...
        $("div.hdrcell")[0].id = "check";

        $("#check").click(function () {
            if (dictDataList.rows.length > 0) {
                
                for (var i = 0; i < dictDataList.rows.length; i++) {
                    if (dictDataList.rows[i].data[0] == 0) {
                        dictDataList.rows[i].data[0] = 1;
                    }
                    else {
    ...
    toolBar.attachEvent("onClick", function (id) {
        switch (id) {
            ...
            case "print":
                ...
                window.open("SalCheckoutPrint.html?ids=" + checked);
                //var rowids = checked.split(',');
                //if (confirm("您确定审核选定的结算单吗?")) {
                //    for (var i = 0; i < rowids.length; i++) {

                //        IBusinessService.auditSettle.statementID = rowids[i];
                //        rock.AjaxRequest(IBusinessService.auditSettle, rock.exceptionFun);
                //        if (!IBusinessService.auditSettle.success) {
                //            alert("审核结算单出错!");
                //            return;
                //        }
                //    }
